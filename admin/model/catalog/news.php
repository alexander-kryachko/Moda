...
class ModelCatalogNews extends Model {
	public function addNews($data, $ifcopy = 0) {
		...
		foreach (@$data['news_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "news_description SET news_id = '" . (int)$news_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', description = '" . $this->db->escape($value['description']) . "', meta_desc = '" . $this->db->escape($value['meta_desc']) . "', meta_key = '" . $this->db->escape($value['meta_key']) . "', ctitle = '" . $this->db->escape($value['ctitle']) . "', ntags = '" . $this->db->escape($value['ntags']) . "', description2 = '" . $this->db->escape($value['description2']) . "', cfield1 = '" . $this->db->escape($value['cfield1']) . "', cfield2 = '" . $this->db->escape($value['cfield2']) . "', cfield3 = '" . $this->db->escape($value['cfield3']) . "', cfield4 = '" . $this->db->escape($value['cfield4']) . "'");
		}
