...
class ModelCatalogncategory extends Model {
	public function addncategory($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "ncategory SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', template_name = '" . $data['template_name'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW()");
	...
	public function editncategory($ncategory_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "ncategory SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', template_name = '" . $data['template_name'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE ncategory_id = '" . (int)$ncategory_id . "'");
