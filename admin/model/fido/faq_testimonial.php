...
class ModelFidoFaqTestimonial extends Model {
	...
	public function editFaq($faq_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "faq_testimonial SET
	...
	public function getFaqs($topic_id) {
		$faq_data = $this->cache->get('faq_testimonial.' . $this->config->get('config_language_id') . '.' . $topic_id);
		if (!$faq_data) {
			$faq_data = array();
			...
			//$this->cache->set('faq_testimonial.' . $this->config->get('config_language_id') . '.' . $topic_id, $faq_data);
		}
		return $faq_data;
	}
