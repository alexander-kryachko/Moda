...
class ModelFidoReviews extends Model {
	...
	public function editFaq($faq_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'reviews_id=" . (int)$faq_id. "'");
		if ($data['keyword']) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET
				query = 'reviews_id=" . (int)$faq_id . "',
				keyword = '" . $this->db->escape($data['keyword']) . "'");
		}
		$this->db->query("DELETE FROM " . DB_PREFIX . "reviews_to_store WHERE faq_id = '" . (int)$faq_id . "'");
		if (isset($data['faq_store'])) {		
			foreach ($data['faq_store'] as $store_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "reviews_to_store SET
					faq_id = '" . (int)$faq_id . "',
					store_id = '" . (int)$store_id . "'");
			}
		}
	...
	public function deleteFaq($faq_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "reviews WHERE faq_id = '" . (int)$faq_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "reviews_description WHERE faq_id = '" . (int)$faq_id . "'");
		$query = $this->db->query("SELECT faq_id FROM " . DB_PREFIX . "reviews WHERE topic_id = '" . (int)$faq_id . "'");
	...
	public function getFaq($faq_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'reviews_id=" . (int)$faq_id . "') AS keyword FROM " . DB_PREFIX . "reviews WHERE faq_id = '" . (int)$faq_id . "'");
		return $query->row;
	} 

	public function getFaqs($topic_id) {
		$faq_data = $this->cache->get('reviews.' . $this->config->get('config_language_id') . '.' . $topic_id);
		if (!$faq_data) {
			$faq_data = array();
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "reviews f
										LEFT JOIN " . DB_PREFIX . "reviews_description fd ON (f.faq_id = fd.faq_id)
											WHERE
	...
	public function getTopic($faq_id) {
		$query = $this->db->query("SELECT title, topic_id FROM " . DB_PREFIX . "reviews f
									LEFT JOIN " . DB_PREFIX . "reviews_description fd ON (f.faq_id = fd.faq_id)
									WHERE
	...
	public function getFaqStores($faq_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "reviews_to_store WHERE faq_id = '" . (int)$faq_id . "'");
		foreach ($query->rows as $result) {
	...
	public function getTotalFaqs() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "reviews");
		return $query->row['total'];
	}

	public function checkFaqs() {
		$create_faq = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "reviews` (`faq_id` int(11) NOT NULL auto_increment, `topic_id` int(11) NOT NULL default '0', `status` int(1) NOT NULL default '0', `sort_order` int(3) NOT NULL default '0', PRIMARY KEY  (`faq_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_bin;";
		$this->db->query($create_faq);
		$create_faq_descriptions = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "reviews_description` (`faq_id` int(11) NOT NULL default '0', `language_id` int(11) NOT NULL default '0', `title` varchar(64) collate utf8_bin NOT NULL default '', `meta_description` varchar(255) collate utf8_bin, `description` text collate utf8_bin NOT NULL, PRIMARY KEY  (`faq_id`,`language_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_bin;";
		$this->db->query($create_faq_descriptions);
		$create_faq_to_store = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "reviews_to_store` (`faq_id` int(11) NOT NULL, `store_id` int(11) NOT NULL, PRIMARY KEY  (`faq_id`, `store_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_bin;";
