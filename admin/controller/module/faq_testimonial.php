...
class ControllerModuleFaqTestimonial extends Controller {
	...
	public function index() {
		$this->load->language('module/faq_testimonial');
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/faq_testimonial', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/faq_testimonial', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('module/faq_testimonial.tpl', $data));
	}
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
			...
			$url = '';
			$this->response->redirect($this->url->link('module/faq_testimonial/listing', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	private function getModule() {
		...
		$data['breadcrumbs'][] = array(
			'href'      => $this->url->link('module/faq_testimonial', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title'),
			'separator' => ' :: '
		);
		...
		$data['faqs'] = $this->url->link('module/faq_testimonial/listing', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
	...
	private function getList() {
		...
		$data['breadcrumbs'][] = array(
			'href'      => $this->url->link('module/faq_testimonial/listing', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title'),
			'separator' => ' :: '
		);

		$data['module'] = $this->url->link('module/faq_testimonial', 'token=' . $this->session->data['token'], 'SSL');
		$data['insert'] = $this->url->link('module/faq_testimonial/insert', 'token=' . $this->session->data['token'], 'SSL');
		$data['delete'] = $this->url->link('module/faq_testimonial/delete', 'token=' . $this->session->data['token'], 'SSL');

		$data['topics'] = array();

		$faq_total = $this->model_fido_faq_testimonial->getTotalFaqs();

		$results = $this->model_fido_faq_testimonial->getFaqs(0);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('module/faq_testimonial/update', 'token=' . $this->session->data['token'] . '&faq_id=' . $result['faq_id'], 'SSL')
			);
		...
		$this->response->setOutput($this->load->view('module/faq_testimonial/list.tpl', $data));
	}
	...
	private function getForm() {
		...
		if (!isset($this->request->get['faq_id'])) {
			$data['action'] = $this->url->link('module/faq_testimonial/insert', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/faq_testimonial/update', 'token=' . $this->session->data['token'] . '&faq_id=' . $this->request->get['faq_id'], 'SSL');
		}

		$data['cancel'] = $this->url->link('module/faq_testimonial/listing', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['topics'] = $this->model_fido_faq_testimonial->getFaqs(0);
		...
		if (isset($this->request->post['faq_store'])) {
			...
			$data['faq_store'] = $this->model_fido_faq_testimonial->getFaqStores($this->request->get['faq_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('module/faq_testimonial/form.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/faq_testimonial')) {
