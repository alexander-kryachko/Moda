...
class ModelAccountCustomer extends Model {
    ...
    public function addCustomerForNews($data) {
        $this->event->trigger('pre.customer.add', $data);
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', firstname = '" . $this->db->escape($data['news_firstname']) . "', lastname = '" . $this->db->escape($data['news_lastname']) . "', email = '" . $this->db->escape($data['news_email']) . "', telephone = '" . $this->db->escape($data['news_telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? serialize($data['custom_field']['account']) : '') . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . (isset($data['news_newsletter']) ? (int)$data['news_newsletter'] : 0) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '1', approved = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW()");
        ...
        $mail->setTo($data['news_email']);
        $mail->setFrom($this->config->get('config_email'));
        $mail->setSender(html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'));
        $mail->setSubject($subject);
        $mail->setText(
            "Здравствуйте, ваш аккаунт создан, для отпивки на новости вы можете зайти в личный кабинет и отписаться!!! Ваш пароль: " . $data['password']
        );
        $mail->send();
