...
(SELECT date_end FROM " . DB_PREFIX . "product_special ps
					...
					ORDER BY ps.priority ASC, ps.price ASC LIMIT 1)
				AS special_end,
(SELECT percent FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS percent,
...
(SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class,
		...
		if ($query->num_rows) {
			return array(
				...
				'table_size'       => $query->row['table_size'],
