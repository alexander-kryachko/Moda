...
class ModelFidoReviews extends Model {
	...
	public function addFaq($data){
		$this->db->query("INSERT INTO ". DB_PREFIX . "reviews SET
			topic_id = '0',
			status = '0',
			sort_order = '0'");
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "reviews_to_store SET
			faq_id = '" . (int)$faq_id . "',
			store_id = '0'");
		$this->cache->delete('reviews');
	}

	public function getTopic($faq_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "reviews f
			LEFT JOIN " . DB_PREFIX . "reviews_description fd ON (f.faq_id = fd.faq_id)
			LEFT JOIN " . DB_PREFIX . "reviews_to_store f2s ON (f.faq_id = f2s.faq_id)
			WHERE
	...
	public function getTotalFaqsByTopicId($topic_id = 0) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "reviews f
			LEFT JOIN " . DB_PREFIX . "reviews_to_store f2s ON (f.faq_id = f2s.faq_id)
			WHERE
				f.topic_id = '" . (int)$topic_id . "' AND
				f2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND
