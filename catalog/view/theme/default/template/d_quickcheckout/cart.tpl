      ...
      <div class="form-horizontal qc-summary <?php if($show_price){ echo 'qc-hide';}?>">
        <?php foreach ($totals as $total) { ?>
          <div class="row qc-totals">
            <label class="col-xs-10 control-label" ><?php echo $total['title']; ?>:</label>
            <div class="col-xs-2 form-control-static"><?php echo $total['text']; ?></div>
          </div>
        <?php } ?>
      </div>
...
<script><!--
  $(function(){
    ...
    $(document).on('click', '.qc-image .fa-trash-o', function() {
      $(this).parent('.qc-image').parent('tr').children('td.qc-quantity').children('div.input-group').children('.qc-product-qantity').val('0');
      refreshAllSteps()
    })
