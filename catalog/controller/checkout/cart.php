...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		$data['go_back'] = $this->language->get('go_back');
		$data['go_home'] = $this->language->get('go_home');
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$data['button_continue_buy'] = $this->language->get('button_continue_buy');

			$data['text_price_unit'] = $this->language->get('text_price_unit');
			$data['text_price_total'] = $this->language->get('text_price_total');
	...
	public function add() {
		...
		if ($product_info) {
			...
			if (!$json) {
				...
				$data['cart_products'][$key]['image'] = $data['cart_products'][$key]['image'] && file_exists(DIR_IMAGE . $data['cart_products'][$key]['image']) ? $this->model_tool_image->resize($data['cart_products'][$key]['image'], 176, 248) : $this->model_tool_image->resize('no_image.jpg', 176, 248);
				...
				$data['button_checkout'] = $this->language->get('button_checkout');
				$data['button_checkout_now'] = $this->language->get('button_checkout_now');
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/cart_popup.tpl')) {
					...
					$json['success'] = $this->load->view('default/template/common/cart_popup.tpl', $data);
				}
