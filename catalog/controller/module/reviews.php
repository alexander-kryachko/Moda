...
class ControllerModuleReviews extends Controller {
	...
	public function index() {
		...
		$data['reviews_title'] = $this->language->get('reviews_title');
		...
		if(($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()){
			...
			$this->response->redirect($this->url->link('information/information', 'information_id=16', 'SSL'));
		}
		...
		$faqs = $this->model_fido_reviews->getTopics(0);

		foreach ($faqs as $faq) {
			$children_data = array();

			$children = $this->model_fido_reviews->getTopics($faq['faq_id']);
			...
			$data['faqs'][] = array(
				...
				'href'			=> $this->url->link('information/reviews', 'topic=' . $faq['faq_id'])
			);
		}
