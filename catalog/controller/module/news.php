...
class ControllerModuleNews extends Controller {
	public function index($setting) {
		...
		$data['text_all_project'] = $this->language->get('text_all_project');
		$data['link_all_project'] = $this->url->link('news/ncategory', 'ncat=61');
		...
		foreach ($results as $result) {
				...
				$href = $this->url->link('news/article', 'ncat=' . $catid['ncategory_id'] . '&news_id=' . $result['news_id']);
		...
		$ncategories = $this->model_catalog_ncategory->getncategories(61);
		...
		foreach ($ncategories as $ncategory) {
			...
			$data['ncategories'][] = array(
				...
				'name'        => $ncategory['name'],
				'thumb'  => $this->model_tool_image->resize($ncategory['image'], 380, 300),
				'description' => utf8_substr(strip_tags(html_entity_decode($ncategory['description'], ENT_QUOTES, 'UTF-8')), 0, 180) . '..',
				'children'    => $children_data,
				'href'        => $this->url->link('news/ncategory', 'ncat=' . $ncategory['ncategory_id'])
			);
		}

		if ( !empty($setting['html_tpl']) ) {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/' . $setting['html_tpl'] . '.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/module/' . $setting['html_tpl'] . '.tpl', $data);
			} else {
				return $this->load->view('default/template/module/' . $setting['html_tpl'] . '.tpl', $setting['html_tpl']);
			}
		} else {
