	...
	public function add() {
		...
		if ($product_info) {
			if (!in_array($this->request->post['product_id'], $this->session->data['wishlist'])) {
				...
				$json['total_in_wishlist'] = $this->language->get('button_wishlist_in');
				if ($this->customer->isLogged()) {
			...
			$json['total'] = sprintf($this->language->get('text_wishlist_count'), (isset($this->session->data['wishlist']) ? count($this->session->data['wishlist']) : 0));
		}
